...
class FinderToolsTest extends Test {
	...
	function testWhich(async: Async) {
		...
		async.branch(branch -> "executable".which({all: false, path: ["test/fixtures"]})
			.then((executable: String) -> Finder.isWindows ? Assert.isTrue(executable.endsWith("\\test\\fixtures\\executable.cmd")) : Assert.fail("Promise not rejected"))
		...
		async.branch(branch -> "executable".which({all: true, path: ["test/fixtures"]})
			.then((executables: Array<String>) -> if (!Finder.isWindows) Assert.fail("Promise not rejected") else {
		...
		async.branch(branch -> "executable.sh".which({all: false, path: ["test/fixtures"]})
			.then((executable: String) -> Finder.isWindows ? Assert.fail("Promise not rejected") : Assert.isTrue(executable.endsWith("/test/fixtures/executable.sh")))
